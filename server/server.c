...
int send_file(char *pathname, int clientsock){
	struct stat fstat, *sp;
	int fd, f, n;
  int size;
  char buffer[MAX];
	sp = &fstat;
	if ((f = lstat(pathname, &fstat)) < 0)
	{
		return;
	}
    size = sp->st_size;
    ...
    write(clientsock, &size, sizeof(size));
    while(n = read(fd, buffer, MAX))
    {
...
int send_file_n(char *pathname, int clientsock, int nlines){
	struct stat fstat, *sp;
	int fd, f, n;
  int size;
  int inc = 0;
  char buffer[MAX];
    ...
    fd = open(pathname, O_RDONLY);
    write(clientsock, &size, sizeof(size));
    while(inc < nlines && n = read(fd, buffer, MAX))
    {
        inc++;
        write(clientsock, buffer, n);
    }
...
int main() 
{
    ...
    while(1){
      ...
      {
        if(!(strlen(input[1]) == 0)){
          send_file(input[1], client_sock);
        }
      }
      //Like get but on n number of lines
      else if (!strcmp(input[0], "put")){
        if (!strlen(input[1] == 0)){
          if (!strlen(input[2] > 0)){
            printf("Copying %d, # of lines from %s", input[2], input[1]);
            send_file_n(input[1], client_sock, input[2]);
          }
        }
      }
